...
test "Tracy allocator supports aligned allocation and reallocation" {
    inline for (.{ null, "test" }) |name| {
        var wrapper = TracyAllocator(name).init(std.testing.allocator);
        const allocator = wrapper.allocator();
        var bytes = try allocator.alignedAlloc(u8, .@"64", 32);
        defer allocator.free(bytes);
        @memset(bytes, 0xa5);
        bytes = try allocator.realloc(bytes, 4096);
        for (bytes[0..32]) |byte| {
